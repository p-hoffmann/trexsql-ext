...
# Session timeline: record skill start (local-only, never sent anywhere)
 '{"skill":"document-release","event":"started","branch":"'"$_BRANCH"'","session":"'"$_SESSION_ID"'"}' 2>/dev/null &
...
```bash
find . -maxdepth 2 -name "*.md" -not -path "./.git/*" -not -path "./node_modules/*" -not -path "./.devx/*" -not -path "./.context/*" | sort
```
...
(adapt to whatever project you're in — these are not ):
...
**If GitHub:**
```bash
gh pr view --json body -q .body > /tmp/-body-$$.md
```

**If GitLab:**
```bash
glab mr view -F json 2>/dev/null | python3 -c "import sys,json; print(json.load(sys.stdin).get('description',''))" > /tmp/-body-$$.md
```
...
**If GitHub:**
```bash
gh pr edit --body-file /tmp/-body-$$.md
```

**If GitLab:**
Read the contents of `/tmp/-body-$$.md` using the Read tool, then pass it to `glab mr update` using a heredoc to avoid shell metacharacter issues:
```bash
