...
# Session timeline: record skill start (local-only, never sent anywhere)
 '{"skill":"design-consultation","event":"started","branch":"'"$_BRANCH"'","session":"'"$_SESSION_ID"'"}' 2>/dev/null &
...
If `BROWSE_NOT_AVAILABLE`: use `open file://...` instead of `use BrowserNavigate, BrowserClick, BrowserScreenshot tools goto` to open
...
MUST be saved to `.devx/projects/$SLUG/designs/`, NEVER to `.context/`,
...
If the browse binary is available (`use BrowserNavigate, BrowserClick, BrowserScreenshot tools` is set), visit the top 3-5 sites in the space and capture visual evidence:

```bash
use BrowserNavigate, BrowserClick, BrowserScreenshot tools goto "https://example-site.com"
use BrowserNavigate, BrowserClick, BrowserScreenshot tools screenshot "/tmp/design-research-site-name.png"
use BrowserNavigate, BrowserClick, BrowserScreenshot tools snapshot
```
...
This phase generates visual previews of the proposed design system. Two paths depending on whether the design tools is available.
...
```bash
 variants --brief "<product name: [name]. Product type: [type]. Aesthetic: [direction]. Colors: primary [hex], secondary [hex], neutrals [range]. Typography: display [font], body [font]. Layout: [approach]. Show a realistic [page type] screen with [specific content for this product].>" --count 3 --output-dir "$_DESIGN_DIR/"
```

Run quality check on each variant:

```bash
 check --image "$_DESIGN_DIR/variant-A.png" --brief "<the original brief>"
```
...
3. Generate new variants with ` iterate` or ` variants` using updated brief
4. Create new board: ` compare --images "..." --output "$_DESIGN_DIR/design-board.html"`
...
**POLLING FALLBACK:** Only use polling if ` serve` fails (no port available).
...
- Use ` extract --image "$_DESIGN_DIR/variant-<CHOSEN>.png"` to analyze the approved mockup and extract design tokens (colors, typography, spacing) that will populate DESIGN.md in Phase 6. This grounds the design system in what was actually approved visually, not just what was described in text.
- If the user wants to iterate further: ` iterate --feedback "<user's feedback>" --output "$_DESIGN_DIR/refined.png"`
