---
name: design-review
aliases: ["design", "design-review", "ui-review"]
version: 2.0.0
description: |
...
```bash
use BrowserNavigate, BrowserClick tools status 2>/dev/null | grep -q "Mode: cdp" && echo "CDP_MODE=true" || echo "CDP_MODE=false"
```
...
[ -z "design tools" ] && D=~/.claude/skills/gstack/design/dist/design
if [ -x "design tools" ]; then
  echo "DESIGN_READY: design tools"
else
  echo "DESIGN_NOT_AVAILABLE"
fi
B=""
[ -n "$_ROOT" ] && [ -x "$_ROOT/.claude/skills/gstack/browse/dist/browse" ] && B="$_ROOT/.claude/skills/gstack/browse/dist/browse"
[ -z "use BrowserNavigate, BrowserClick tools" ] && B=~/.claude/skills/gstack/browse/dist/browse
if [ -x "use BrowserNavigate, BrowserClick tools" ]; then
  echo "BROWSE_READY: use BrowserNavigate, BrowserClick tools"
else
...
If `BROWSE_NOT_AVAILABLE`: use `open file://...` instead of `use BrowserNavigate, BrowserClick tools` to open
comparison boards. The user just needs to see the HTML file in any browser.

If `DESIGN_READY`: the design binary is available for visual mockup generation.
Commands:
- `design tools generate --brief "..." --output /path.png` — generate a single mockup
- `design tools variants --brief "..." --count 3 --output-dir /path/` — generate N style variants
- `design tools compare --images "a.png,b.png,c.png" --output /path/board.html --serve` — comparison board + HTTP server
- `design tools serve --html /path/board.html` — serve comparison board and collect feedback via HTTP
- `design tools check --image /path.png --brief "..."` — vision quality gate
- `design tools iterate --session /path/session.json --feedback "..." --output /path.png` — iterate

**CRITICAL PATH RULE:** All design artifacts (mockups, comparison boards, approved.json)
MUST be saved to ``, NEVER to `.context/`,
...
2. Take a full-page desktop screenshot: `use BrowserNavigate, BrowserClick tools "$REPORT_DIR/screenshots/first-impression.png"`
...
use BrowserNavigate, BrowserClick tools js "JSON.stringify([...new Set([...document.querySelectorAll('*')].slice(0,500).map(e => getComputedStyle(e).fontFamily))])"

# Color palette in use
use BrowserNavigate, BrowserClick tools js "JSON.stringify([...new Set([...document.querySelectorAll('*')].slice(0,500).flatMap(e => [getComputedStyle(e).color, getComputedStyle(e).backgroundColor]).filter(c => c !== 'rgba(0, 0, 0, 0)'))])"

# Heading hierarchy
use BrowserNavigate, BrowserClick tools js "JSON.stringify([...document.querySelectorAll('h1,h2,h3,h4,h5,h6')].map(h => ({tag:h.tagName, text:h.textContent.trim().slice(0,50), size:getComputedStyle(h).fontSize, weight:getComputedStyle(h).fontWeight})))"

# Touch target audit (find undersized interactive elements)
use BrowserNavigate, BrowserClick tools js "JSON.stringify([...document.querySelectorAll('a,button,input,[role=button]')].filter(e => {const r=e.getBoundingClientRect(); return r.width>0 && (r.width<44||r.height<44)}).map(e => ({tag:e.tagName, text:(e.textContent||'').trim().slice(0,30), w:Math.round(e.getBoundingClientRect().width), h:Math.round(e.getBoundingClientRect().height)})).slice(0,20))"

# Performance baseline
use BrowserNavigate, BrowserClick tools
```
...
```bash
use BrowserNavigate, BrowserClick tools <url>
use BrowserNavigate, BrowserClick tools -i -a -o "$REPORT_DIR/screenshots/{page}-annotated.png"
use BrowserNavigate, BrowserClick tools responsive "$REPORT_DIR/screenshots/{page}"
use BrowserNavigate, BrowserClick tools console --errors
use BrowserNavigate, BrowserClick tools
```
...
- `text-wrap: balance` or `text-pretty` on headings (check via `use BrowserNavigate, BrowserClick tools css <heading> text-wrap`)
...
- `prefers-reduced-motion` respected (check: `use BrowserNavigate, BrowserClick tools js "matchMedia('(prefers-reduced-motion: reduce)').matches"`)
...
```bash
use BrowserNavigate, BrowserClick tools -i
use BrowserNavigate, BrowserClick tools @e3           # perform action
use BrowserNavigate, BrowserClick tools -D          # diff to see what changed
```
...
11. **Show screenshots to the user.** After every `use BrowserNavigate, BrowserClick tools`, `use BrowserNavigate, BrowserClick tools -a -o`, or `use BrowserNavigate, BrowserClick tools responsive` command, use the Read tool on the output file(s) so the user can see them inline. For `responsive` (3 files), Read all three. This is critical — without it, screenshots are invisible to the user.
...
```bash
design tools generate --brief "<description of the page/component with the finding fixed, referencing DESIGN.md constraints>" --output "$REPORT_DIR/screenshots/finding-NNN-target.png"
```
...
```bash
use BrowserNavigate, BrowserClick tools <affected-url>
use BrowserNavigate, BrowserClick tools "$REPORT_DIR/screenshots/finding-NNN-after.png"
use BrowserNavigate, BrowserClick tools console --errors
use BrowserNavigate, BrowserClick tools -D
```
...
2. If target mockups were generated during the fix loop AND `DESIGN_READY`: run `design tools verify --mockup "$REPORT_DIR/screenshots/finding-NNN-target.png" --screenshot "$REPORT_DIR/screenshots/finding-NNN-after.png"` to compare the fix result against the target. Include pass/fail in the report.
...
```bash
```
Write a one-line summary to `` with a pointer to the full report in `$REPORT_DIR`.
