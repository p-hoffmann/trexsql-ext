...
aliases: ["plan-design", "design-plan-review"]
version: 2.0.0
description: |
...
- `mkdir -p `
- `design tools generate`, `design tools variants`, `design tools compare`, `design tools iterate`, `design tools evolve`, `design tools check`
- `open` (fallback for viewing boards when `use BrowserNavigate, BrowserClick tools` is not available)
...
```bash
design tools variants --brief "<description assembled from plan + DESIGN.md constraints>" --count 3 --output-dir "$_DESIGN_DIR/"
```

After generation, run a cross-model quality check on each variant:

```bash
design tools check --image "$_DESIGN_DIR/variant-A.png" --brief "<the original brief>"
```
...
```bash
design tools compare --images "$_DESIGN_DIR/variant-A.png,$_DESIGN_DIR/variant-B.png,$_DESIGN_DIR/variant-C.png" --output "$_DESIGN_DIR/design-board.html" --serve
```
...
3. Generate new variants with `design tools iterate` or `design tools variants` using updated brief
4. Create new board: `design tools compare --images "..." --output "$_DESIGN_DIR/design-board.html"`
...
**POLLING FALLBACK:** Only use polling if `design tools serve` fails (no port available).
...
**If `DESIGN_NOT_AVAILABLE`:** Tell the user: "The gstack designer isn't set up yet. Run `design tools setup` to enable visual mockups. Proceeding with text-only review, but you're missing the best part." Then proceed to review passes with text-based review.
...
```bash
design tools generate --brief "<description of what 10/10 looks like for this dimension>" --output /tmp/<dimension>.png
```
...
* **NEVER use AskUserQuestion to ask which variant the user prefers.** Always create a comparison board first (`design tools compare --serve`) and open it in the browser. The board has rating controls, comments, remix/regenerate buttons, and structured feedback output. Use AskUserQuestion ONLY to notify the user the board is open and wait for them to finish — not to present variants inline and ask "which do you prefer?" That is a degraded experience.
...
|----------------|-------------|-----------|-------|
| [screen name]  |  | [brief description] | [constraints from review] |
```
...
already writes to `` and `` — this is
the same pattern. The review dashboard depends on this data. Skipping this
command breaks the review readiness dashboard in /ship.

```bash
 '{"skill":"plan-design-review","timestamp":"TIMESTAMP","status":"STATUS","initial_score":N,"overall_score":N,"unresolved":N,"decisions_made":N,"commit":"COMMIT"}'
```
