...
# Session timeline: record skill start (local-only, never sent anywhere)
 '{"skill":"health","event":"started","branch":"'"$_BRANCH"'","session":"'"$_SESSION_ID"'"}' 2>/dev/null &
...
Append one JSONL line to `.devx/projects/$SLUG/health-history.jsonl`:
...
Read the last 10 entries from `.devx/projects/$SLUG/health-history.jsonl` (if the
