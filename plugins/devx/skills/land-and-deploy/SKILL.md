...
echo "design toolsEPLOY_CONFIG"

# If config exists, parse it
if [ "design toolsEPLOY_CONFIG" != "NO_CONFIG" ]; then
  PROD_URL=$(echo "design toolsEPLOY_CONFIG" | grep -i "production.*url" | head -1 | sed 's/.*: *//')
  PLATFORM=$(echo "design toolsEPLOY_CONFIG" | grep -i "platform" | head -1 | sed 's/.*: *//')
...
CURRENT_HASH=$(sed -n '/## Deploy Configuration/,/^## /p' CLAUDE.md 2>/dev/null | shasum -a 256 | cut -d' ' -f1)
WORKFLOW_HASH=$(find .github/workflows -maxdepth 1 \( -name '*deploy*' -o -name '*cd*' \) 2>/dev/null | xargs cat 2>/dev/null | shasum -a 256 | cut -d' ' -f1)
echo "${CURRENT_HASH}-${WORKFLOW_HASH}" > 
```
Continue to Step 2.
...
ls -t ~/./evals/*-e2e-*-$(date +%Y-%m-%d)*.json 2>/dev/null | head -20
```
...
```bash
setopt +o nomatch 2>/dev/null || true  # zsh compat
ls -t ~/./evals/*-llm-judge-*-$(date +%Y-%m-%d)*.json 2>/dev/null | head -5
```
...
Then run `` to classify the changes:

```bash
eval $( $(gh pr view --json baseRefName -q .baseRefName 2>/dev/null || echo main) 2>/dev/null)
...
| SCOPE_CONFIG only | Smoke: `use BrowserNavigate, BrowserClick tools` + verify 200 status |
...
```bash
use BrowserNavigate, BrowserClick tools -i -a -o ".gstack/deploy-reports/post-deploy.png"
```
