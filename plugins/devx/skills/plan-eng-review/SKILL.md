...
# Session timeline: record skill start (local-only, never sent anywhere)
 '{"skill":"plan-eng-review","event":"started","branch":"'"$_BRANCH"'","session":"'"$_SESSION_ID"'"}' 2>/dev/null &
...
DESIGN=$(ls -t .devx/projects/$SLUG/*-$BRANCH-design-*.md 2>/dev/null | head -1)
[ -z "$DESIGN" ] && DESIGN=$(ls -t .devx/projects/$SLUG/*-design-*.md 2>/dev/null | head -1)
...
Read the `/office-hours` skill file at ` using the Read tool.
...
Write to `.devx/projects/{slug}/{user}-{branch}-eng-review-test-plan-{datetime}.md`:
...
`.devx/` (user config directory, not project files). The skill preamble
already writes to `.devx/sessions/` and `.devx/analytics/` — this is
the same pattern. The review dashboard depends on this data. Skipping this
command breaks the review readiness dashboard in /ship.

```bash
 '{"skill":"plan-eng-review","timestamp":"TIMESTAMP","status":"STATUS","unresolved":N,"critical_gaps":N,"issues_found":N,"mode":"MODE","commit":"COMMIT"}'
```
...
**Review tiers:**
- **Eng Review (required by default):** The only review that gates shipping. Covers architecture, code quality, tests, performance. Can be disabled globally with \` set skip_eng_review true\` (the "don't bother me" setting).
