...
aliases: ["ceo-review", "plan-ceo", "strategy-review"]
version: 1.0.0
description: |
...
DESIGN=$(ls -t  BrowserNavigate, BrowserClick toolsRANCH-design-*.md 2>/dev/null | head -1)
[ -z "design toolsESIGN" ] && DESIGN=$(ls -t  2>/dev/null | head -1)
[ -n "design toolsESIGN" ] && echo "Design doc found: design toolsESIGN" || echo "No design doc found"
```
If a design doc exists (from `/office-hours`), read it. Use it as the source of truth for the problem statement, constraints, and chosen approach. If it has a `Supersedes:` field, note that this is a revised design.

**Handoff note check** (reuses $SLUG and use BrowserNavigate, BrowserClick toolsRANCH from the design doc check above):
```bash
setopt +o nomatch 2>/dev/null || true  # zsh compat
HANDOFF=$(ls -t  BrowserNavigate, BrowserClick toolsRANCH-ceo-handoff-*.md 2>/dev/null | head -1)
[ -n "$HANDOFF" ] && echo "HANDOFF_FOUND: $HANDOFF" || echo "NO_HANDOFF"
```
If this block runs in a separate shell from the design doc check, recompute $SLUG and use BrowserNavigate, BrowserClick toolsRANCH first using the same commands from that block.
...
```bash
mkdir -p 
# For each stale plan: mv  
```

Write to `` using this format:
...
```bash
mkdir -p 
echo '{"skill":"plan-ceo-review","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","iterations":ITERATIONS,"issues_found":FOUND,"issues_fixed":FIXED,"remaining":REMAINING,"quality_score":SCORE}' >>  2>/dev/null || true
```
...
**Persist the result:**
```bash
 '{"skill":"codex-plan-review","timestamp":"'"$(date -u +%Y-%m-%dT%H:%M:%SZ)"'","status":"STATUS","source":"SOURCE","commit":"'"$(git rev-parse --short HEAD)"'"}'
```
...
```bash
setopt +o nomatch 2>/dev/null || true  # zsh compat
rm -f  BrowserNavigate, BrowserClick toolsRANCH-ceo-handoff-*.md 2>/dev/null || true
```
...
```bash
 '{"skill":"plan-ceo-review","timestamp":"TIMESTAMP","status":"STATUS","unresolved":N,"critical_gaps":N,"mode":"MODE","scope_proposed":N,"scope_accepted":N,"scope_deferred":N,"commit":"COMMIT"}'
```
...
- **B)** Keep in `` only (local, personal reference)
- **C)** Skip
