...
use crate::logging::FlightLogger;
...
/// Arrow Flight service backed by a shared DuckDB connection.
#[derive(Clone)]
...
impl DuckDBFlightService {
    ...
    /// Execute a SQL query against DuckDB and collect the resulting
...
impl FlightService for DuckDBFlightService {
    ...
    async fn handshake(
        ...
        FlightLogger::debug("handshake", &format!("Handshake request from client, node_id={node_id}"));
    ...
    /// Return one FlightInfo per DuckDB table with schema and endpoint.
    async fn list_flights(
        &self,
        _request: Request<Criteria>,
    ) -> Result<Response<Self::ListFlightsStream>, Status> {
        FlightLogger::debug("list_flights", &format!("Listing tables on {}:{}", self.host, self.port));
        ...
        FlightLogger::debug("list_flights", &format!("Returning {} flights", flights.len()));
    ...
    async fn do_get(
        ...
        FlightLogger::info("do_get", &format!("Executing query on {}:{}", self.host, self.port));
        FlightLogger::debug("do_get", &format!("SQL: {sql}"));
        let connection = self.connection.clone();
    ...
    async fn list_actions(
        ...
        let actions = vec![ActionType {
            ...
            description: "Execute a SQL statement (DDL/DML) against DuckDB".to_string(),
        }];
...
pub fn start_flight_server(
    ...
    FlightLogger::info("server", &format!("Starting flight server on {host}:{port} (tls={tls_enabled})"));
    ...
    FlightLogger::info("server", &format!("Flight server started successfully on {host}:{port}"));
...
pub fn start_flight_server_with_tls(
    ...
    FlightLogger::info("server", &format!("Starting flight server with mTLS on {host}:{port}"));

    // Read and validate certs eagerly to surface errors before spawning.
    ...
    FlightLogger::info("server", &format!("Flight server with mTLS started successfully on {host}:{port}"));
...
pub fn stop_flight_server(host: &str, port: u16) -> Result<String, String> {
    FlightLogger::info("server", &format!("Stopping flight server on {host}:{port}"));
    let result = ServerRegistry::instance().stop_server(host, port);
    if result.is_ok() {
        FlightLogger::info("server", &format!("Flight server stopped on {host}:{port}"));
    }
    result
}
