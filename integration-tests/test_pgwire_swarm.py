...
def test_swarm_start_pgwire_service(node_factory):
    """swarm_start_service('pgwire', json_config) starts server and registers in gossip."""
    node = node_factory(load_pgwire=True, load_flight=True, load_swarm=True)
    ...
    # Use swarm_start_service to start pgwire with JSON config
    node.execute(
        f"""SELECT swarm_start_service('pgwire', '{{"host": "127.0.0.1", "port": {node.pgwire_port}, "password": "test"}}')"""
    )
