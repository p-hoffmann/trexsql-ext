"""Circe standalone tests.

Verifies that the circe extension can load and execute OHDSI Circe functions:
...
def test_circe_load_and_hello(node_factory):
    """Extension loads and circe_hello() returns greeting."""
    node = node_factory(load_circe=True, load_flight=False, load_swarm=False)
    result = node.execute("SELECT circe_hello('Sam')")
    assert len(result) == 1
    assert result[0][0] == "Circe Sam"


def test_circe_openssl_version(node_factory):
    """circe_openssl_version() reports linked OpenSSL version."""
    node = node_factory(load_circe=True, load_flight=False, load_swarm=False)
    result = node.execute("SELECT circe_openssl_version('Test')")
    assert len(result) == 1
    assert "OpenSSL" in result[0][0]


def test_circe_sql_render(node_factory):
    """circe_sql_render() substitutes @-parameters into SQL template."""
    node = node_factory(load_circe=True, load_flight=False, load_swarm=False)
    result = node.execute(
...
def test_circe_sql_translate(node_factory):
    """circe_sql_translate() converts SQL Server syntax to target dialect."""
    node = node_factory(load_circe=True, load_flight=False, load_swarm=False)
    result = node.execute(
...
def test_circe_sql_render_translate(node_factory):
    """circe_sql_render_translate() renders parameters then translates dialect."""
    node = node_factory(load_circe=True, load_flight=False, load_swarm=False)
    result = node.execute(
...
def test_circe_json_to_sql(node_factory):
    """circe_json_to_sql() converts base64 OHDSI cohort JSON to SQL."""
    node = node_factory(load_circe=True, load_flight=False, load_swarm=False)
    result = node.execute(
        f"SELECT circe_json_to_sql('{COHORT_B64}', '{OPTIONS_JSON}')"
    )
    assert len(result) == 1
