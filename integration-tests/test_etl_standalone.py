...
def test_etl_status_empty(node_factory):
    """etl_status() returns empty result when no pipelines are running."""
    node = node_factory(load_etl=True, load_flight=False, load_swarm=False)
...
def test_etl_stop_nonexistent(node_factory):
    """etl_stop() returns error for a pipeline that does not exist."""
    node = node_factory(load_etl=True, load_flight=False, load_swarm=False)
    with pytest.raises(RuntimeError, match="not found"):
        node.execute("SELECT etl_stop('nonexistent')")


def test_etl_start_missing_publication(node_factory):
    """etl_start() returns error when connection string lacks publication."""
    node = node_factory(load_etl=True, load_flight=False, load_swarm=False)
    with pytest.raises(RuntimeError, match="publication"):
        node.execute(
            "SELECT etl_start('test_pipe', "
